...
//__________________________________________________________________________________________________________
void makeSinglePhotonSpectraFitsFromJanusTree_modified(  TString fileName     = "", 
    ...
    if (runnumber > -1)
      ...
      // outputDir = Form("%s/Run_with_%s/Run%05d",outputDir.Data(), BGSMOOTH.Data(), runnumber );

    TString outputDirPlots = Form("%s/Plots",outputDir.Data());
    // TString outputDirPlots = Form("%s/Run_with_%s/Plots",outputDir.Data(), BGSMOOTH.Data() );
      ...
      for (Int_t i = 0; i < gMaxChannels; i++){
        ...
        if (currentRunInfo.vop < 3.5){
          histNSHG_BG[j][i]                   = (TH1D*)spectrumFitter[j][i]->Background(histNSHG[j][i], 14, "smoothing3");
           ...
           histNSHG_BG[j][i]                   = (TH1D*)spectrumFitter[j][i]->Background(histNSHG[j][i], 14, BGSMOOTH);
        }
        ...
        if (currentRunInfo.vop < 3.5){
          nSPE[j][i] = spectrumFitter[j][i]->Search(histNSHG_Sub_mapped[layer][chBoard], 7., "", 0.005); // Search for peaks
        } else {
          nSPE[j][i] = spectrumFitter[j][i]->Search(histNSHG_Sub_mapped[layer][chBoard], 7., "", 0.005); // Search for peaks
        }
        ...
        if (nSPE[j][i] > 2){
          ...
          // fitMultGaussNSHG[j][i] = new TF1(Form("fitMultGauss_NS_HG_Sub_B%d_C%02d",j,i), multGauss, ( - 20), ( + 20), 3 * nPeaksMultGauss+2);
